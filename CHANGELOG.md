...
### Added
- **帐号配置自动同步**：首页 IP 筛选面板新增「同步到帐号配置」选项。勾选后点击生成配置，系统会将您选中的地区直接保存到该 Token 的后台配置中，使得订阅链接在无参数的情况下也能下发对应地区的节点。

### Fixed
- **代码健壮性**：重构了 GitHub Commit 逻辑为通用方法，支持在非管理页面进行安全的数据回写。

## [v1.7.1] - 2026-02-26

### Fixed
- **订阅链接地区筛选失效**：修复首页 IP 筛选生成配置时未将 `regions` 参数附加到订阅链接的问题。现在生成的链接会自动包含 `?regions=...` 参数，确保客户端订阅的节点与用户选择一致。
